...
import argparse
import torchaudio
from meta.wham_room import WhamRoom
...
def create_rirs(output_dir, sr=8000):
    ...
    SPLITS = ["tr","cv","tt"]
    [os.makedirs(os.path.join(output_dir,splt), exist_ok=True) for splt in SPLITS]
        ...
        for output_name in tqdm(utt_ids):
            ...
            for i, mics in enumerate(rir):
                for j, source in enumerate(mics):
                    ...
                    torchaudio.save(
                        os.path.join(
                            output_dir, splt, "{}_{}_".format(i, j) + output_name,
                        ),
                        h_torch,
                        sr,
                    )
